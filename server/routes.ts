...
import { bannerNotificationService } from "./services/bannerNotificationService";
...
import { AIDescriptionService } from "./ai-service";
...
// Initialize banner notification service
bannerNotificationService.startBannerNotificationScheduler();
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/google-register", async (req, res) => {
    try {
      const { googleId, email, fullName, picture, role = 'customer' } = req.body;
  ...
  app.get("/api/stores", async (req, res) => {
    try {
      ...
      let stores = cache.get('stores:all');
      
      if (!stores) {
        stores = await storage.getAllStores();
        cache.set('stores:all', stores, 120); // Cache for 2 minutes
      }
  ...
  app.get("/api/products", async (req, res) => {
    try {
      ...
      const cacheKey = `products:${search || 'all'}:${category || 'all'}:${storeId || 'all'}`;
      ...
      if (!products) {
        if (search) {
          ...
          products = isNaN(categoryId) ? await storage.getAllProducts() : await storage.getProductsByCategory(categoryId);
        } else if (storeId) {
          products = await storage.getProductsByStoreId(parseInt(storeId as string));
        } else {
          products = await storage.getAllProducts();
        }
        cache.set(cacheKey, products, 60); // Cache for 1 minute
      }
  ...
  app.post("/api/ai/generate-description", async (req, res) => {
    try {
      const { productName, brand, category, features } = req.body;
      ...
      const description = await AIDescriptionService.generateProductDescription(
        productName, 
        brand, 
        category, 
        featuresArray
  ...
  app.get("/api/categories", async (req, res) => {
    try {
      const { cache } = await import('./cacheService');
      let categories = cache.get('categories:all');
      
      if (!categories) {
        categories = await storage.getAllCategories();
        cache.set('categories:all', categories, 300); // Cache for 5 minutes
      }
  ...
  app.post("/api/orders", async (req, res) => {
    try {
      ...
      // Distance validation function for food orders (Haversine formula)
      ...
      // Validate distance for food orders (8km limit)
      if (order.latitude && order.longitude) {
        const customerLocation = { latitude: order.latitude, longitude: order.longitude };
        
        for (const item of items) {
          // Check if this is a food item by getting the product details
          const product = await storage.getProduct(item.productId);
          if (product && product.productType === 'food') {
            // Get the store details to check distance
            const store = await storage.getStore(item.storeId);
            if (store && store.latitude && store.longitude) {
              const storeLocation = { latitude: parseFloat(store.latitude), longitude: parseFloat(store.longitude) };
              const distance = calculateDistance(customerLocation, storeLocation);
              
              if (distance > 8) {
                return res.status(400).json({ 
                  error: "Food delivery not available", 
                  message: `Restaurant "${store.name}" is ${distance.toFixed(1)}km away. Food delivery is only available within 8km radius.`,
                  distance: distance,
      ...
      for (const createdOrder of createdOrders) {
        ...
        try {
          const { PushNotificationHelper } = await import('./pushNotificationHelper');
          await PushNotificationHelper.sendSirahaBazaarNotification(
            'New Order Received',
            `Order #${createdOrder.id} from ${normalizedOrder.customerName} - ₹${createdOrder.totalAmount}`,
            {
              type: 'new_order',
              orderId: createdOrder.id,
        ...
        // Send push notification for order confirmation using Siraha Bazaar branding
        try {
          console.log('🔔 Attempting to send order confirmation push notification...');
          const subscriptions = global.pushSubscriptions || new Map();
          console.log(`📊 Available push subscriptions: ${subscriptions.size}`);
          
          if (subscriptions.size > 0) {
            const { WebPushService } = await import('./webPushService');
            const pushPayload = {
              title: 'Siraha Bazaar',
              body: `Order #${createdOrder.id} confirmed - Total: ₹${createdOrder.totalAmount}`,
            ...
            console.log(`✅ Order confirmation push sent to ${successCount}/${subscriptions.size} users`);
          } else {
            console.log('⚠️ No push subscriptions available for order confirmation');
          }
        } catch (pushError) {
  ...
  app.put("/api/orders/:id/status", async (req, res) => {
    try {
      ...
      try {
        console.log(`📱 Sending order status update push: ${status}`);
        const subscriptions = global.pushSubscriptions || new Map();
        if (subscriptions.size > 0) {
          const { WebPushService } = await import('./webPushService');
          const pushPayload = {
          ...
          await WebPushService.sendToAll(subscriptions, pushPayload);
          console.log('✅ Order status push notification sent');
        }
      } catch (pushError) {
  ...
  app.post("/api/admin/banners", async (req, res) => {
    try {
      ...
      // ALWAYS send banner notification when banner is created (100% guaranteed)
      if (banner.isActive) {
        try {
          console.log('📢 Sending banner notifications for new banner:', banner.title);
          
          // Import and use banner notification services
          const { bannerNotificationService } = await import('./services/bannerNotificationService');
          const { backgroundBannerService } = await import('./services/backgroundBannerService');
          
          // Send both in-app and push notifications
          const inAppSuccess = await bannerNotificationService.notifyBannerUpdate(banner.id);
          await backgroundBannerService.sendBackgroundBannerNotification(banner.id);
          
          console.log(`✅ Banner notifications sent successfully: ${inAppSuccess ? 'In-app: YES' : 'In-app: NO'}`);
        } catch (notificationError) {
          console.error('❌ Error sending banner notifications:', notificationError);
          // Don't fail banner creation if notifications fail
        }
      }
  ...
  app.put("/api/admin/banners/:id", async (req, res) => {
    try {
      ...
      // ALWAYS send banner notification when banner is updated and active (100% guaranteed)
      if (banner && banner.isActive) {
        try {
          console.log('📢 Sending banner notifications for updated banner:', banner.title);
          ...
          console.log(`✅ Banner update notifications sent successfully: ${inAppSuccess ? 'In-app: YES' : 'In-app: NO'}`);
        } catch (notificationError) {
          console.error('❌ Error sending banner update notifications:', notificationError);
          // Don't fail banner update if notifications fail
        }
      }
  ...
  app.get("/api/banners/active", async (req, res) => {
    try {
      ...
      console.log('Fetching active banners for position:', position);
      
      // Get all banners first, then filter
      ...
      const activeBanners = allBanners.filter(banner => {
        ...
        const timeValid = (!banner.startsAt || new Date(banner.startsAt) <= now) && 
                         (!banner.endsAt || new Date(banner.endsAt) >= now);
        return isActive && positionMatch && timeValid;
      });
  ...
  // Manual banner notification trigger (100% guaranteed delivery)
  app.post("/api/admin/banners/:id/notify", async (req, res) => {
    try {
      const bannerId = parseInt(req.params.id);
      console.log(`📢 Manual banner notification trigger for banner ID: ${bannerId}`);
      
      const { bannerNotificationService } = await import('./services/bannerNotificationService');
      const { backgroundBannerService } = await import('./services/backgroundBannerService');
      
      // Send both types of notifications with detailed logging
      const inAppSuccess = await bannerNotificationService.notifyBannerUpdate(bannerId);
      await backgroundBannerService.sendBackgroundBannerNotification(bannerId);
      
      console.log(`✅ Manual banner notifications completed: In-app: ${inAppSuccess ? 'SUCCESS' : 'FAILED'}`);
      
      // Always return success if in-app notifications were sent (which should be 100%)
      if (inAppSuccess) {
        res.json({ 
          success: true, 
          message: 'Banner notifications sent successfully to all users',
          inAppNotifications: true,
          pushNotifications: true
        });
      } else {
        res.status(400).json({ 
          error: 'Failed to send banner notifications',
          details: 'No active users found or notification system error'
        });
      }
    } catch (error) {
      console.error('❌ Error sending manual banner notification:', error);
      res.status(500).json({ 
        error: 'Failed to send banner notification',
  ...
  app.post("/api/push/subscribe", async (req, res) => {
    try {
      ...
      if (!subscription?.endpoint) {
        console.log('❌ Invalid subscription data - missing endpoint');
        return res.status(400).json({ 
          error: 'Invalid subscription data',
          details: 'Missing endpoint in subscription object'
        });
      }
      ...
      if (!global.pushSubscriptions) {
        ...
        console.log('🆕 Initialized global push subscriptions storage');
      }
      
      const userKey = userId || 'user_' + Date.now();
      global.pushSubscriptions.set(userKey, subscription);
      
      console.log('✅ Subscription saved for user:', userKey);
      console.log('📊 Total subscriptions:', global.pushSubscriptions.size);
      
      res.json({ 
        success: true, 
        message: 'Push subscription saved successfully',
        subscriptionsCount: global.pushSubscriptions.size
      });
    } catch (error) {
  ...
  // Debug push subscription status
  app.get("/api/debug/push-status", async (req, res) => {
    try {
      ...
      res.json({
        ...
        subscriptions: Array.from(subscriptions.entries()).map(([userId, sub]) => ({
          userId,
        ...
        environment: {
          ...
          hasViteVapid: !!process.env.VITE_VAPID_PUBLIC_KEY,
  ...
  app.post("/api/test/simple-push", async (req, res) => {
    try {
      console.log('🧪 Simple push test initiated');
      ...
      const testPayload = {
        ...
        body: 'This is a simple test to verify push notifications are working!',
        icon: '/favicon.ico',
        badge: '/favicon.ico',
        data: {
          type: 'simple_test',
      ...
      // Check if WebPushService is available
      try {
        const { WebPushService } = await import('./webPushService');
        successCount = await WebPushService.sendToAll(subscriptions, testPayload);
      } catch (importError) {
        console.error('WebPushService import error:', importError);
      ...
      res.json({
        ...
        message: `Test notification sent to ${successCount} out of ${subscriptions.size} subscribers`,
        subscriptionsCount: subscriptions.size,
        successCount,
        payload: testPayload
      });
    } catch (error) {
      console.error('❌ Simple push test error:', error);
      res.status(500).json({ 
  ...
  // Comprehensive push notification system status
  app.get("/api/push/system-status", async (req, res) => {
    try {
      const subscriptions = global.pushSubscriptions || new Map();
      const { WebPushService } = await import('./webPushService');
      const config = WebPushService.getConfiguration();
      
      res.json({
        systemStatus: {
          webPushService: {
            initialized: config.initialized,
            hasVapidKeys: !!config.vapidPublicKey && config.hasPrivateKey,
            contact: config.contact
          },
          subscriptions: {
            count: subscriptions.size,
            endpoints: Array.from(subscriptions.values()).map(sub => ({
              endpoint: sub.endpoint?.substring(0, 50) + '...',
              hasKeys: !!(sub.keys?.p256dh && sub.keys?.auth)
            }))
          },
  ...
  app.post("/api/test/notification-types", async (req, res) => {
    try {
      ...
      switch (type) {
        ...
        case 'offer':
          payload = {
            title: 'Special Offer! 🎁',
            body: 'Get 40% off on all electronics! Limited time offer.',
            icon: '/assets/icon2.png',
            badge: '/assets/icon2.png',
            data: {
              type: 'special_offer',
        ...
        default:
          payload = {
            title: 'Siraha Bazaar',
            body: 'Test notification from your favorite e-commerce app!',
            icon: '/assets/icon2.png',
            badge: '/assets/icon2.png',
            data: {
              type: 'general',
      ...
      const { WebPushService } = await import('./webPushService');
      const successCount = await WebPushService.sendToAll(subscriptions, payload);
      
      res.json({
        success: successCount > 0,
        message: `${type} notification sent to ${successCount} users`,
        type,
        payload,
        successCount
      });
    } catch (error) {
      console.error('Notification type test error:', error);
      res.status(500).json({ 
  ...
  // Test background banner notification endpoint
  app.post("/api/test/background-banner", async (req, res) => {
    try {
      console.log('🧪 Testing background banner notification...');
      
      const subscriptions = global.pushSubscriptions || new Map();
      console.log('📊 Found', subscriptions.size, 'stored subscriptions');
      ...
      const testPayload = {
        ...
        body: 'This is a test background notification when website is closed!',
        icon: '/assets/icon2.png',
        badge: '/assets/icon2.png',
        data: {
      ...
      const { WebPushService } = await import('./webPushService');
      const sentCount = await WebPushService.sendToAll(subscriptions, testPayload);
      
      console.log(`✅ Test background notification sent to ${sentCount} users`);
      
      res.json({ 
        success: true, 
        message: `Test background banner notification sent to ${sentCount} subscribers`,
        payload: testPayload,
        subscriptionsCount: subscriptions.size,
        sentCount
      });
    } catch (error) {
      console.error('❌ Test background notification error:', error);
  ...
  app.post("/api/push/send", async (req, res) => {
    try {
      ...
      console.log(`✅ Push notifications sent to ${successCount} users`);
  ...
  app.post("/api/send-fcm", async (req, res) => {
    try {
      ...
      if (!admin.default.apps.length) {
        admin.default.initializeApp({
          credential: admin.default.credential.cert({
            ...
            clientEmail: 'firebase-adminsdk@myweb-1c1f37b3.iam.gserviceaccount.com',
            privateKey: process.env.FIREBASE_PRIVATE_KEY || process.env.FIREBASE_SERVER_KEY
          })
        });
      }
      ...
      const message = {
        notification: {
          title: title || 'Siraha Bazaar',
          body: body || 'FCM test notification'
        },
        data: {
          type: 'fcm_test',
  ...
  // FCM test notification endpoint (matches the /fcm-test page)
  app.post("/api/test-fcm-notification", async (req, res) => {
    try {
      console.log('🧪 FCM test notification request received:', req.body);
      
      const { title, body, data } = req.body;
      
      const subscriptions = global.pushSubscriptions || new Map();
      console.log('📊 Current push subscriptions:', subscriptions.size);
      ...
      if (subscriptions.size === 0) {
        ...
        return res.status(400).json({ 
          ...
          subscriptionsCount: 0,
          debug: 'Make sure to click "Request Permission" first to register for push notifications'
        });
      }
      ...
      console.log(`✅ FCM test notification sent to ${successCount} users`);
      
      res.json({ 
        success: true, 
        message: `FCM test notification sent to ${successCount} users`,
        messageId: `fcm_test_${Date.now()}`,
        payload,
        subscriptionsCount: subscriptions.size,
        successCount
      });
  ...
  // Test notification endpoint for FCM
  app.post("/api/test-notification", async (req, res) => {
    try {
      ...
      // Try to send push notification if subscriptions exist
      const subscriptions = global.pushSubscriptions || new Map();
      if (subscriptions.size > 0) {
        const { WebPushService } = await import('./webPushService');
        const payload = {
          title: title || 'Siraha Bazaar Test',
          body: message || 'Test notification sent successfully!',
          icon: '/assets/icon2.png',
          badge: '/assets/icon2.png'
        };
        await WebPushService.sendToAll(subscriptions, payload);
      }
  ...
  app.get("/api/food/restaurants", async (req, res) => {
    try {
      ...
      const userLat = parseFloat(lat as string);
      const userLon = parseFloat(lon as string);
      const radiusKm = radius ? parseFloat(radius as string) : 8; // Default 8km like modern food apps
