...
export interface IStorage {
  ...
  savePushSubscription(userId: string, subscription: any): Promise<boolean>;
  getPushSubscriptions(): Promise<Map<string, any>>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getNotificationsByUserId(userId: number): Promise<Notification[]> {
    try {
      ...
      console.error(`Error in getNotificationsByUserId for user ${userId}:`, error);
      return [];
    }
  }
  ...
  async savePushSubscription(userId: string, subscription: any): Promise<boolean> {
    try {
      // Store in global memory for now (in production, use database)
  ...
  async getPushSubscriptions(): Promise<Map<string, any>> {
    return global.pushSubscriptions || new Map();
  }
}
