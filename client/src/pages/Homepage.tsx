...
export default function Homepage() {
  ...
  // Scroll loading state for products
  ...
  const allFeaturedProducts = recommendedProducts.length > 0 
  ...
  const featuredProducts = allFeaturedProducts.slice(0, visibleProductCount);

  // Scroll handler for loading more products on scroll down
  useEffect(() => {
    const handleScroll = () => {
      if (isLoadingMoreProducts || visibleProductCount >= allFeaturedProducts.length) return;
      ...
      if (scrollTop + windowHeight >= docHeight - 200) {
        ...
        setTimeout(() => {
          setVisibleProductCount(prev => Math.min(prev + 4, allFeaturedProducts.length));
  ...
  }, [isLoadingMoreProducts, visibleProductCount, allFeaturedProducts.length]);
  ...
  return (
    <PageLoader minLoadTime={600}>
      ...
      <BannerDisplay position="main" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4" />
      ...
      {/* Popular Stores - Only show after all products are loaded */}
      {!isProductsLoading && featuredProducts.length > 0 && visibleProductCount >= allFeaturedProducts.length && (
      ...
      {/* SirahaBazaar Loader just above bottom navbar - mobile compatible */}
      {isLoadingMoreProducts && (
        <div className="fixed bottom-16 md:bottom-4 left-1/2 transform -translate-x-1/2 z-50">
