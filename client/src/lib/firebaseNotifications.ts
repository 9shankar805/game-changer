  ...
  try {
    ...
    if (!messaging && 'serviceWorker' in navigator) {
      try {
        ...
        console.log('✅ Firebase service worker active and ready');
      } catch (error: any) {
        console.log('⚠️ Firebase not supported in this browser, using fallback');
      ...
      if (Notification.permission === 'granted') {
        try {
          ...
          if (!navigator.serviceWorker.controller) {
            console.log('⚠️ Service worker not controlling, reloading...');
          ...
          console.log('⚠️ FCM token generation skipped:', tokenError.message);
        }
      } else {
