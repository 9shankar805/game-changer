...
// import { initPushNotifications } from "@/lib/hardcodedPush";
import "@/lib/pushNotificationManager";
...
import CompletePushTest from "@/pages/CompletePushTest";
import SirahaBazaarLoader from "@/components/SirahaBazaarLoader";
  ...
  return (
    <div className="relative">
      <Switch>
        ...
        <Route path="/push-test" component={PushTest} />
        <Route path="/complete-push-test" component={CompletePushTest} />
...
function App() {
  ...
  useEffect(() => {
    ...
    // Also initialize comprehensive web push notifications
    setTimeout(async () => {
      console.log('🚀 Initializing comprehensive push notification system...');
      
      // Initialize hardcoded push system
      // const hardcodedSuccess = await initPushNotifications();
      // console.log('📱 Hardcoded push system:', hardcodedSuccess ? 'SUCCESS' : 'FAILED');
      
      // Initialize comprehensive push manager
      if ((window as any).pushManager) {
        const managerSuccess = await (window as any).pushManager.initialize();
        console.log('🔧 Push manager system:', managerSuccess ? 'SUCCESS' : 'FAILED');
        
        if (managerSuccess) {
          console.log('🎉 All push notification systems initialized successfully!');
          
          // Optional: Send a welcome notification
          setTimeout(() => {
            (window as any).pushManager.showLocalNotification(
              'Siraha Bazaar',
              'Push notifications are now active! You\'ll receive order updates and special offers.',
              ...
              <AppModeProvider>
                ...
                <TooltipProvider>
                  <ErrorBoundary>
                    ...
                    <Toaster />
                    <PWAInstallPrompt />
                    <SirahaBazaarLoader isLoading={isLoading} />
                  </ErrorBoundary>
                </TooltipProvider>
                </SoundProvider>
              </AppModeProvider>
